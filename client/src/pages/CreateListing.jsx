...
import { FiUploadCloud, FiX, FiPlus, FiHome, FiDollarSign } from 'react-icons/fi';
  ...
  const handleImageSubmit = () => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch(() => { setImageUploadError('Échec du téléchargement (2 MB max par image)'); setUploading(false); });
    ...
    <div style={{ background: 'var(--surface)', minHeight: '100vh', paddingTop: '72px' }}>
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '48px 24px' }}>
        ...
        <div style={{ marginBottom: '36px' }}>
          <div className='section-label'><FiPlus size={12} /> Nouvelle annonce</div>
          <h1 style={{ fontWeight: 800, fontSize: '2rem', letterSpacing: '-0.02em' }}>Créer une annonce</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-7">
          ...
          <div className='glass-card' style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
            ...
            <input type='text' id='name' placeholder='Nom de la propriété' maxLength='62' minLength='10' required
              onChange={handleChange} value={formData.name} style={inputStyle} onFocus={focusInput} onBlur={blurInput} />
            <textarea id='description' placeholder='Description détaillée...' required rows={4}
              onChange={handleChange} value={formData.description} style={inputStyle} onFocus={focusInput} onBlur={blurInput} />
            <input type='text' id='address' placeholder='Adresse complète' required
              onChange={handleChange} value={formData.address} style={inputStyle} onFocus={focusInput} onBlur={blurInput} />
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                {checkboxData.map(({ id, label, icon }) => {
                  const isActive = id === 'sale' ? formData.type === 'sale'
                    : id === 'rent' ? formData.type === 'rent'
                    : formData[id];
                  return (
                    <label key={id} style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <input type='number' id='bedrooms' min='1' max='10' required value={formData.bedrooms}
                    onChange={handleChange} style={numInputStyle} onFocus={focusInput} onBlur={blurInput} />
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                    <FaBed size={14} style={{ color: '#818cf8' }} /> Chambres
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <input type='number' id='bathrooms' min='1' max='10' required value={formData.bathrooms}
                    onChange={handleChange} style={numInputStyle} onFocus={focusInput} onBlur={blurInput} />
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                    <FaBath size={14} style={{ color: '#818cf8' }} /> Salles de bain
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <input type='number' id='regularPrice' min='50' max='10000000' required value={formData.regularPrice}
                    onChange={handleChange} style={numInputStyle} onFocus={focusInput} onBlur={blurInput} />
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                    <FiDollarSign size={14} style={{ color: '#818cf8' }} />
                    <div>
                      <div>Prix normal</div>
                      {formData.type === 'rent' && <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>/mois</div>}
                    </div>
                  </div>
                </div>
                {formData.offer && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <input type='number' id='discountPrice' min='0' max='10000000' required value={formData.discountPrice}
                      onChange={handleChange} style={numInputStyle} onFocus={focusInput} onBlur={blurInput} />
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                      <FaPercent size={12} style={{ color: '#f59e0b' }} />
                      <div>
                        <div>Prix réduit</div>
                        {formData.type === 'rent' && <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>/mois</div>}
                      </div>
          ...
          {/* Right col - Images */}
          <div className='glass-card' style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
              Images <span style={{ color: 'var(--text-muted)', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(max 6, première = couverture)</span>
            </p>
            ...
            >
              <FiUploadCloud size={36} style={{ color: '#818cf8', margin: '0 auto 12px' }} />
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '12px' }}>
                Sélectionnez vos photos
              </p>
              <input onChange={(e) => setFiles(e.target.files)} type='file' id='images' accept='image/*' multiple
                style={{ display: 'block', margin: '0 auto', color: 'var(--text-muted)', fontSize: '0.82rem', cursor: 'pointer' }} />
            </div>
            ...
            {imageUploadError && (
              <div style={{ padding: '10px 14px', borderRadius: '8px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#f87171', fontSize: '0.85rem' }}>
                {imageUploadError}
              </div>
            )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
                {formData.imageUrls.map((url, index) => (
                  <div key={url} style={{ position: 'relative', borderRadius: '10px', overflow: 'hidden', aspectRatio: '4/3' }}>
                    ...
                    {index === 0 && (
                      <span style={{
                        position: 'absolute', top: '6px', left: '6px', background: 'rgba(79,70,229,0.85)',
                        color: '#fff', fontSize: '0.65rem', fontWeight: 700, padding: '2px 8px', borderRadius: '50px',
                      }}>Couverture</span>
                    )}
                    <button type='button' onClick={() => handleRemoveImage(index)} style={{
                      ...
                      width: '26px', height: '26px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
            ...
            <button type='submit' disabled={loading || uploading} className='btn-primary'
              style={{ width: '100%', justifyContent: 'center', marginTop: 'auto', opacity: (loading || uploading) ? 0.7 : 1 }}>
              <FiPlus size={16} />
